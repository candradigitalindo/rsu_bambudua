...
@section('content')
<div class="card mb-3">
  <div class="card-header"><h5 class="card-title">Laporan Cash Flow</h5></div>
  <div class="card-body">
    <form method="GET" action="{{ route('keuangan.laporan.cash-flow') }}" class="row g-2 align-items-end">
      <div class="col-md-3">
      ...
      <div class="col-md-6 d-flex gap-2">
        ...
        <a class="btn btn-outline-danger" href="{{ route('keuangan.laporan.cash-flow.pdf') . '?' . http_build_query(request()->query()) }}">PDF</a>
        <a class="btn btn-outline-success" href="{{ route('keuangan.laporan.cash-flow.excel') . '?' . http_build_query(request()->query()) }}">Excel</a>
      </div>
    </form>
  </div>
</div>
