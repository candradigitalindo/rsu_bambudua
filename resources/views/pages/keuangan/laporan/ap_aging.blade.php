@extends('layouts.app')
@section('title','Laporan AP Aging')
@section('content')
<div class="card mb-3">
  <div class="card-header"><h5 class="card-title">Laporan AP Aging</h5></div>
  <div class="card-body">
    <form method="GET" action="{{ route('keuangan.laporan.ap-aging') }}" class="row g-2 align-items-end">
      <div class="col-md-3">
        <label class="form-label">Per {{ date('Y') }}</label>
      ...
      <div class="col-md-6 d-flex gap-2">
        ...
        <a class="btn btn-outline-danger" href="{{ route('keuangan.laporan.ap-aging.pdf') . '?' . http_build_query(request()->query()) }}">PDF</a>
        <a class="btn btn-outline-success" href="{{ route('keuangan.laporan.ap-aging.excel') . '?' . http_build_query(request()->query()) }}">Excel</a>
      </div>
    </form>
  </div>
</div>
