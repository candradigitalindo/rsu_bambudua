...
@section('content')
    <div class="container-fluid px-4 py-4">
        ...
        <div class="page-header-card mb-4">
            ...
            <p class="text-muted mb-0">Monitoring dan statistik layanan laboratorium</p>
        </div>
                ...
                <div class="stat-card border-info">
                    <div class="stat-content">
                        <div class="stat-info">
                            ...
                            <div class="stat-value text-info">{{ $stats['requested'] ?? 0 }}</div>
                        </div>
        ...
        <div class="row g-4 mb-4">
            <div class="col-md-6">
                <a href="{{ route('lab.reagents.index', ['filter' => 'habis']) }}" class="text-decoration-none">
                    <div class="stat-card border-danger clickable">
                        <div class="stat-content">
                            <div class="stat-info">
                                <div class="stat-label">Stok Reagent Habis</div>
                                <div class="stat-value text-danger">{{ $reagentStats['habis'] ?? 0 }}</div>
            ...
            <div class="col-md-6">
                <a href="{{ route('lab.reagents.index', ['filter' => 'kadaluarsa']) }}" class="text-decoration-none">
                    <div class="stat-card border-warning clickable">
                        <div class="stat-content">
                            <div class="stat-info">
                                <div class="stat-label">Reagent Kadaluarsa</div>
                                <div class="stat-value text-warning">{{ $reagentStats['kadaluarsa'] ?? 0 }}</div>
                                <small class="text-muted">Klik untuk lihat detail</small>
                            </div>
                            <div class="stat-icon bg-warning">
            ...
            <div class="card-header bg-white border-0 py-3">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('lab.requests.index') }}" class="btn btn-sm btn-outline-primary">
                        <i class="ri-external-link-line me-1"></i> Lihat Semua
                    </a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse($recent as $r)
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-primary">{{ $r->encounter->rekam_medis ?? '-' }}</span>
                                </td>
                                <td>
                                    <div class="fw-semibold">{{ $r->encounter->name_pasien ?? '-' }}</div>
                                </td>
                                <td>
                                    @php
                                        ...
                                        $statusIcon = match ($r->status) {
                                            'requested' => 'ri-file-list-line',
                                            'collected' => 'ri-test-tube-line',
                                ...
                                <td>
                                    <span class="fw-semibold text-success">Rp
                                        {{ number_format($r->total_charge, 0, ',', '.') }}</span>
                                </td>
                                <td>
                                    <div class="d-flex gap-1">
                                        <a href="{{ route('lab.requests.show', $r->id) }}"
                                            class="btn btn-sm btn-outline-primary" title="Detail">
                                            <i class="ri-eye-line"></i>
                                        </a>
                                        <a href="{{ route('lab.requests.edit', $r->id) }}" class="btn btn-sm btn-primary"
                                            title="Edit Hasil">
