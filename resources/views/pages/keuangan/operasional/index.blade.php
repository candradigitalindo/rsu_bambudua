        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title">Data Pengeluaran Operasional</h5>
                </div>
                <div class="card-body">
                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
                        <a href="{{ route('operasional.create') }}" class="btn btn-primary">
                            <i class="ri-add-line"></i> Tambah Pengeluaran
                        </a>
                        <form action="{{ route('operasional.index') }}" method="GET" class="d-flex flex-wrap gap-2">
                            ...
                            <tbody>
                                @forelse ($expenses as $index => $expense)
                                    <tr>
                                        ...
                                        <td>{{ \Carbon\Carbon::parse($expense->expense_date)->translatedFormat('d F Y') }}
                                        </td>
                                        <td>{{ $expense->description }}</td>
                                        <td class="text-end">{{ formatPrice($expense->amount) }}</td>
                                        <td>
                                            <div class="d-inline-flex gap-1">
                                                <a href="{{ route('operasional.edit', $expense->id) }}"
                                                    class="btn btn-warning btn-sm">
                                                    <i class="ri-edit-line"></i>
                                                </a>
                                                <form action="{{ route('operasional.destroy', $expense->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
