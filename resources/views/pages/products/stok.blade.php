            ...
            <div class="card-body">
                <form action="{{ route('product.storeStock', $product->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('stok') is-invalid @enderror" id="stok" name="stok" value="{{ old('stok', $product->stok ?? 0) }}" min="0" required>
                        @error('stok')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" rows="2" placeholder="Tulis keterangan jika perlu...">{{ old('keterangan', $product->keterangan ?? '') }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('products.index') }}" class="btn btn-secondary" id="btnKembaliStok">
                            <span class="spinner-border spinner-border-sm d-none" id="spinnerKembaliStok" role="status" aria-hidden="true"></span>
                            <span id="textKembaliStok">Kembali</span>
                        </a>
                        <button type="submit" class="btn btn-primary" id="btnSimpanStok">
                            <span class="spinner-border spinner-border-sm d-none" id="spinnerSimpanStok" role="status" aria-hidden="true"></span>
