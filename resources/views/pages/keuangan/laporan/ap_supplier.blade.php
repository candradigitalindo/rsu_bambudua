@extends('layouts.app')
@section('title','Laporan AP / Supplier')
@section('content')
<div class="card mb-3">
  <div class="card-header">
    <h5 class="card-title">Laporan AP / Supplier</h5>
  </div>
  <div class="card-body">
    <form method="GET" action="{{ route('keuangan.laporan.ap-supplier') }}" class="row g-2 align-items-end">
      <div class="col-md-3">
      ...
      <div class="col-md-3 d-flex gap-2">
        ...
        <a class="btn btn-outline-danger" href="{{ route('keuangan.laporan.ap-supplier.pdf') . '?' . http_build_query(request()->query()) }}">PDF</a>
        <a class="btn btn-outline-success" href="{{ route('keuangan.laporan.ap-supplier.excel') . '?' . http_build_query(request()->query()) }}">Excel</a>
      </div>
    </form>
  </div>
</div>
  ...
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-bordered align-middle m-0">
        ...
        <tbody>
          <tr>
            <td colspan="4" class="text-center text-muted">Belum ada data. Silakan atur filter lalu tekan Terapkan.</td>
          </tr>
        </tbody>
      </table>
    </div>
