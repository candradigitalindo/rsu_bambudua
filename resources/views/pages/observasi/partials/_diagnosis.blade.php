...
(function(){
  ...
  const deleteUrl = function(id){ return "{{ route('observasi.deleteDiagnosis', ':id') }}".replace(':id', id); };
  const icdAjaxUrl = "{{ route('observasi.getIcd10', $observasi) }}";

  function initIcdSelect(){
    if (!$.fn.select2) return;
    $('#icd10_id').select2({
      placeholder: 'Cari kode atau nama diagnosis...', allowClear: true, width: '100%',
      ajax: {
        url: icdAjaxUrl, dataType: 'json', delay: 250,
        ...
        processResults: function(data){
          ...
          return { results: rows.map(function(item){
            ...
            // Fallback
            return { id: item.id || item.code || '', text: (item.description || item.text || '') };
          })};
        }, cache: true
      }
    });
  ...
  function loadDiagnosis(){
    $.ajax({ url: listUrl, type: 'GET', data: { _token: "{{ csrf_token() }}" } })
      .done(function(data){
  ...
  $(document).on('click', '#btn-diagnosis-medis', function(){
    ...
    if (!icd10_id) { alert('Jenis Diagnosis tidak boleh kosong'); return; }
    if (!diagnosis_type) { alert('Tipe Diagnosis tidak boleh kosong'); return; }
    $.ajax({
      url: postUrl, type: 'POST', data: { icd10_id: icd10_id, diagnosis_type: diagnosis_type, _token: "{{ csrf_token() }}" },
      beforeSend: function(){ $('#spinner-diagnosis-medis').removeClass('d-none'); $('#text-diagnosis-medis').addClass('d-none'); }
    }).done(function(resp){
      if (resp.status == 200){ swal(resp.message, { icon: 'success' }); loadDiagnosis(); }
      else { swal('Terjadi kesalahan saat menyimpan data.', { icon: 'error' }); }
    }).always(function(){ $('#spinner-diagnosis-medis').addClass('d-none'); $('#text-diagnosis-medis').removeClass('d-none'); });
  });
