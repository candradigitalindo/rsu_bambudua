        ...
        <div class="col-12 col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form method="POST"
                        action="{{ $mode === 'create' ? route('professional-licenses.store') : route('professional-licenses.update', $license->id) }}">
                        @csrf
                        @if ($mode === 'edit')
                            @method('PUT')
                        @endif
                        ...
                        <div class="mb-3">
                            ...
                            <select name="user_id" class="form-select" required>
                                ...
                                @foreach ($users as $u)
                                    <option value="{{ $u->id }}" @selected(old('user_id', $license->user_id) === $u->id)>{{ $u->name }}
                                        @if ($u->username)
                                            ({{ $u->username }})
                                        @endif
                                    </option>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="profession" class="form-select" required>
                                ...
                                @foreach ($professions as $p)
                                    <option value="{{ $p }}" @selected(old('profession', $license->profession) === $p)>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="sip_number" class="form-control"
                                value="{{ old('sip_number', $license->sip_number) }}" placeholder="Contoh: SIP-123/ABC">
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Tanggal Kadaluarsa</label>
                            <input type="date" name="sip_expiry_date" class="form-control" required
                                value="{{ old('sip_expiry_date', optional($license->sip_expiry_date)->format('Y-m-d')) }}">
                        </div>
