              ...
              <tr>
                ...
                <td>
                  <div class="d-flex flex-wrap align-items-center gap-1">
                    <a href="{{ route('radiologi.requests.show', $r->id) }}" class="btn btn-outline-secondary btn-sm">Detail</a>
                    @php($st = $r->status)
                    @if($st === 'requested')
                      <a href="{{ route('radiologi.requests.schedule.create', $r->id) }}" class="btn btn-outline-primary btn-sm">Jadwalkan</a>
                      <form method="POST" action="{{ route('radiologi.requests.status', $r->id) }}" class="d-inline-block m-0">
                        @csrf
                        <input type="hidden" name="status" value="canceled">
                        <button type="submit" class="btn btn-outline-danger btn-sm">Batalkan</button>
                      </form>
                    @elseif($st === 'processing')
                      <a href="{{ route('radiologi.requests.results.edit', $r->id) }}" class="btn btn-outline-success btn-sm">Input Hasil</a>
                    @endif
                  </div>
                </td>
              </tr>
