@extends('layouts.app')
@section('title', 'Edit Paket Pemeriksaan')
...
@php
    $selectedTindakan = old('tindakan_items') ? array_keys(old('tindakan_items')) : $paket->getItemIds('tindakan_ids');
    $selectedLab = old('lab_items') ? array_keys(old('lab_items')) : $paket->getItemIds('lab_ids');
    $selectedRadiologi = old('radiologi_items') ? array_keys(old('radiologi_items')) : $paket->getItemIds('radiologi_ids');
    $selectedObat = old('obat_items') ? array_keys(old('obat_items')) : $paket->getItemIds('obat_ids');

    $qtyTindakan = old('tindakan_items') ?? $paket->getItemQtyMap('tindakan_ids');
    $qtyLab = old('lab_items') ?? $paket->getItemQtyMap('lab_ids');
    $qtyRadiologi = old('radiologi_items') ?? $paket->getItemQtyMap('radiologi_ids');
    $qtyObat = old('obat_items') ?? $paket->getItemQtyMap('obat_ids');
@endphp

@section('content')
    <form action="{{ route('paket-pemeriksaan.update', $paket->id) }}" method="POST" id="formPaket">
        @csrf
        @method('PUT')
        <div class="row gx-3">
            {{-- Form Section --}}
                ...
                <div class="card mb-3">
                    <div class="card-header">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <h5 class="card-title mb-0"><i class="ri-edit-2-line me-1"></i> Edit Paket: {{ $paket->name }}</h5>
                        </div>
                    </div>
                    <div class="card-body">
                        {{-- Informasi Dasar --}}
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" name="name" id="inputName"
                                            value="{{ old('name', $paket->name) }}" placeholder="Contoh: Paket Fisioterapi 10 Sesi">
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" class="form-control" name="jumlah_sesi" id="inputSesi"
                                            value="{{ old('jumlah_sesi', $paket->jumlah_sesi) }}" min="1">
                                    </div>
                            ...
                            <div class="col-lg-3 col-sm-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="status">
                                        <option value="1" {{ old('status', $paket->status) == 1 ? 'selected' : '' }}>Aktif</option>
                                        <option value="0" {{ old('status', $paket->status) == 0 ? 'selected' : '' }}>Nonaktif</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" inputmode="numeric" class="form-control @error('harga') is-invalid @enderror"
                                            name="harga" id="harga" value="{{ number_format(old('harga', $paket->harga), 0, ',', '.') }}">
                                        @error('harga')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="mt-2">
                                        <label class="gratis-switch" id="gratisToggle">
                                            <input class="form-check-input" type="checkbox" name="is_gratis"
                                                value="1" id="isGratis" {{ old('is_gratis', $paket->is_gratis) ? 'checked' : '' }}>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" class="form-control" name="masa_berlaku_hari" id="inputBerlaku"
                                            value="{{ old('masa_berlaku_hari', $paket->masa_berlaku_hari) }}" min="1">
                        ...
                        <ul class="nav nav-pills isian-tabs mb-3" role="tablist">
                            <li class="nav-item"><a class="nav-link active" data-bs-toggle="pill" href="#tabTindakan"><i class="ri-stethoscope-line me-1"></i>Tindakan <span class="tab-badge" id="badgeTindakan">{{ count($selectedTindakan) }}</span></a></li>
                            <li class="nav-item"><a class="nav-link" data-bs-toggle="pill" href="#tabLab"><i class="ri-test-tube-line me-1"></i>Lab <span class="tab-badge" id="badgeLab">{{ count($selectedLab) }}</span></a></li>
                            <li class="nav-item"><a class="nav-link" data-bs-toggle="pill" href="#tabRadiologi"><i class="ri-body-scan-line me-1"></i>Radiologi <span class="tab-badge" id="badgeRadiologi">{{ count($selectedRadiologi) }}</span></a></li>
                            <li class="nav-item"><a class="nav-link" data-bs-toggle="pill" href="#tabObat"><i class="ri-capsule-line me-1"></i>Obat <span class="tab-badge" id="badgeObat">{{ count($selectedObat) }}</span></a></li>
                        </ul>
                                ...
                                <select class="form-select item-select" id="tindakanSelect" multiple>
                                    @foreach ($tindakans as $tindakan)
                                        <option value="{{ $tindakan->id }}"
                                            ...
                                            {{ in_array($tindakan->id, $selectedTindakan) ? 'selected' : '' }}>
                                ...
                                <select class="form-select item-select" id="labSelect" multiple>
                                    @foreach ($labs as $lab)
                                        <option value="{{ $lab->id }}"
                                            ...
                                            {{ in_array($lab->id, $selectedLab) ? 'selected' : '' }}>
                                ...
                                <select class="form-select item-select" id="radiologiSelect" multiple>
                                    @foreach ($radiologis as $rad)
                                        <option value="{{ $rad->id }}"
                                            ...
                                            {{ in_array($rad->id, $selectedRadiologi) ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="description" rows="3"
                                placeholder="Keterangan paket (opsional)...">{{ old('description', $paket->description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xl-4 col-lg-4">
                <div class="preview-card" id="previewCard">
                    ...
                    <div class="preview-value" id="prevName">{{ $paket->name }}</div>
                    <div class="preview-divider"></div>
                    <div class="d-flex justify-content-between">
                        <div>
                            <div class="preview-title">JUMLAH SESI</div>
                            <div class="preview-value" id="prevSesi">{{ $paket->jumlah_sesi }} sesi</div>
                        </div>
                        <div class="text-end">
                            <div class="preview-title">MASA BERLAKU</div>
                            <div class="preview-value" id="prevBerlaku">{{ $paket->masa_berlaku_hari }} hari</div>
                        </div>
                    </div>
                    <div class="preview-divider"></div>
                    <div class="text-center">
                        <div class="preview-title">HARGA</div>
                        <div class="preview-price" id="prevHarga">
                            {{ $paket->is_gratis ? 'GRATIS' : 'Rp ' . number_format($paket->harga, 0, ',', '.') }}
                        </div>
                    </div>
                    <div class="preview-divider"></div>
...
$(document).ready(function() {
    ...
    var initQty = {
        tindakan: @json($qtyTindakan),
        lab: @json($qtyLab),
        radiologi: @json($qtyRadiologi),
        obat: @json($qtyObat)
    };
