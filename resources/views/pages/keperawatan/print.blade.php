...
<div class="print-container">
  ...
  <div class="mb-2"><strong>RM:</strong> {{ $record->encounter->rekam_medis ?? '-' }} — <strong>Pasien:</strong> {{ $record->encounter->name_pasien ?? '-' }}</div>
  <div class="mb-2"><strong>Perawat:</strong> {{ $record->nurse->name ?? '-' }} — <strong>Shift:</strong> {{ $record->shift ?? '-' }}</div>
  <div class="mb-2"><strong>Tanda Vital:</strong> SBP {{ $record->systolic ?? '-' }}, DBP {{ $record->diastolic ?? '-' }}, HR {{ $record->heart_rate ?? '-' }}, RR {{ $record->resp_rate ?? '-' }}, Temp {{ $record->temperature ?? '-' }}, SpO2 {{ $record->spo2 ?? '-' }}, Nyeri {{ $record->pain_scale ?? '-' }}</div>
